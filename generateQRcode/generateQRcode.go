...
func init() {
	flag.StringVar(&text, "text", "", "二维码内容")
	flag.StringVar(&logo, "logo", "", "二维码Logo(png)")
	flag.IntVar(&percent, "percent", 15, "二维码Logo的显示比例(默认15%)")
	flag.IntVar(&size, "size", 256, "二维码的大小(默认256)")
	flag.StringVar(&out, "out", "", "输出文件")
	flag.StringVar(&imgText, "bottom", "", "底部文字")
}
